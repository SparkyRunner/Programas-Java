...
public static int[] MergeSort(int [] A, int opt){
  ...
  if(opt == 0){
    ...
    return MergeInsertMain(A, Temp, 0, A.length-1);  
  }else{
    System.out.printf("\n No valid option, using Normal MergeSort");
    return MergeMain(A, Temp, 0, A.length-1);
  }
}
...
public static int [] MergeInsertMain(int [] A, int [] T, int esq, int dir){
    int meio;
    if( esq < dir){
    
        meio = (esq + dir) / 2;
        MergeInsertMain(A, T, esq, meio);
        MergeInsertMain(A, T, meio + 1, dir);
...
public static int[] InsertionSort(int [] A,int esq, int dir) {
    int i, j, chave;
    for(j = esq; j < dir; j++){
        chave = A[j];
        i = j - 1;
