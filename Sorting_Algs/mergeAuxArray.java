...
public static int[] MergeSort(int [] A, int opt){
  ...
  if(opt == 0){
      ...
      return MergeMainWhitouAux(A, Temp, 0, A.length-1);
  }


}

public static int [] MergeMainWhitouAux(int [] A, int [] T, int esq, int dir){
  int meio;
  if (esq < dir){
    
        meio = (esq + dir) / 2;
        MergeMainWhitouAux(A, T, esq, meio);
        MergeMainWhitouAux(A, T, meio + 1, dir);

        MergeWhitoutAux(A, T, esq, meio+1, dir);
        MergeWhitoutAux(T, A, esq, meio+1, dir);
  }
  return A;
}

private static void MergeWhitoutAux(int []A, int [] T, int esqPos, int dirPos, int dirFim){
  int esqFim = dirPos -1;
  int tempPos = esqPos;
